...
```python
from humanize import judge, format_judge_report
...
```python
from humanize import combined_score
...
```python
from humanize import ngram_score
...
`generate.py` 自动:
1. 在 prompt build 时拼入规则段(代替原内嵌的 V4 规则)
2. Step 4 (可选): 对生成的文章跑 detect, 如分数 ≥ 25 自动调 LLM 润色

```bash
# 自动: 截图 + thinking + 文章 + verify + humanize
uv run python scripts/generate.py templeluck.com --two-step --humanize
```
...
```
humanize/
├── __init__.py          API: score / build_humanize_prompt / postprocess_humanize / judge / ngram_score / combined_score
├── patterns.json        规则数据(JSON,可外部修改, 6 大类规则)
...
├── postprocess.py       后处理润色(调 LLM)
├── judge.py             Layer 3 LLM 终审(7 字段结构化 JSON, 防共谋)
└── README.md            本文档

scripts/
├── humanize.py          统一 CLI: detect / prompt / rewrite / compare / judge / combined
└── release_gate.py      5 维发布门控(调用本模块 + verify.py)
```
